...
export default function Signup() {
    ...
    useEffect(() => {
        if (password === confirmPassword && password !== "" && confirmPassword !== "" && fullName !="" && email != "") {
    ...
    return <div className="flex flex-col gap-4  items-center justify-center w-full min-h-screen py-4">
        ...
        <div className="flex gap-4  flex-col border border-gray-300 shadow-md shadow-blue-200  bg-white rounded-2xl px-6 py-8  ">
            ...
            <div>
                <Input heading="Full Name" placeholder="Enter your full name " type="string" onChange={setFullName} />
                <Input heading="Email " placeholder="Enter your email " type="string" onChange={setEmail} />
                <Input heading="Phone Number " placeholder="Phone with country code like +1" type="string" onChange={setMobile} />
                <Input heading="Password " placeholder="Enter your password  " type="password" onChange={setPassword} />
                <Input heading="Confirm password " placeholder="Enter your password  " type="password" onChange={setConfirmPassword} />
            </div>
            ...
            className={` ${passwordMatch ? "cursor-pointer hover:bg-[#253ceb] bg-[#2563EB] " : "pointer-events-none cursor-not-allowed  bg-blue-400"}  text-white transition-all  duration-300 p-3 rounded-2xl`}>{ }Sign up </button>)}
            
            <div className="text-gray-600 flex gap-2 justify-center text-lg ">
                Already have an account ? 
                <div className="hover:underline hover:text-blue-500 text-blue-400 hover:cursor-pointer transition-all duration-300 " onClick={()=>{navigate('/signin',{replace: true})}}>
                    Signin 
                </div>
            </div>
